...
table = (
    ...
    ( 'CMakeLists.txt'
        , r'set\W+value_ptr_lite_version\W+"([0-9]+\.[0-9]+\.[0-9]+)"\W+$'
        , 'set( value_ptr_lite_version "{major}.{minor}.{patch}" )\n' )

#    , ( 'example/cmake-pkg/CMakeLists.txt'
#        , r'set\W+value_ptr_lite_version\W+"([0-9]+\.[0-9]+(\.[0-9]+)?)"\W+$'
#        , 'set( value_ptr_lite_version "{major}.{minor}" )\n' )
#
...
#        , 'value_ptr_lite_version = "{major}.{minor}.{patch}"\n' )
    ...
    , ( 'include/nonstd/value_ptr.hpp'
        , r'\#define\s+value_ptr_lite_MINOR\s+[0-9]+\s*$'
        , '#define value_ptr_lite_MINOR  {minor}' )

    , ( 'include/nonstd/value_ptr.hpp'
        , r'\#define\s+value_ptr_lite_PATCH\s+[0-9]+\s*$'
        , '#define value_ptr_lite_PATCH  {patch}\n' )
)
